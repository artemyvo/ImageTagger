...
- `reasoning` is the chain-of-thought trace that preceded it. It is stored specifically to support **thinking-enabled VLMs** (e.g. models trained with reasoning traces such as Qwen3-VL). Including the CoT in training teaches the model to reason before answering, not just to reproduce captions. It comes from the `THOUGHT:` section of the Vision response; when the Description thinking switch is on and the response has no such section, the model's native thinking trace (Ollama `thinking`, OpenAI-compatible `reasoning_content`) is stored instead.
